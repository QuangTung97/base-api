...
func HTMLGet[T any, Req any](
	...
	r.mux.Get(pattern.GetPattern(), func(writer http.ResponseWriter, request *http.Request) {
		...
		if err := doAssignParams(ctx, &req, pattern); err != nil {
			r.writeResponse(ctx, "", err, http.StatusBadRequest)
			return
		}

		respBody, err := handler(ctx, req)
		r.writeResponse(ctx, respBody, err, http.StatusInternalServerError)
	})
}
...
func (r *Router) writeResponse(ctx *Context, respBody template.HTML, err error, status int) {
	writer := ctx.Writer
